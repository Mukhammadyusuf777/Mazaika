...
let AntigravityService = AntigravityService_1 = class AntigravityService {
    ...
    async generateFullProject(promptText, chatHistory = [], currentConfig, targetEntity = 'bot_and_mini_app') {
        ...
        throw new common_1.InternalServerErrorException('No AI API keys configured. Please set GOOGLE_AI_STUDIO_KEY or CLOUDFLARE_ACCOUNT_ID + CLOUDFLARE_API_TOKEN in .env');
    }
    buildSitePrompt(historyContext, currentConfigContext) {
...
### RESPONSE FORMAT — Return ONLY this valid JSON (no markdown, no backticks):
{
  ...
  "project_data": {
    ...
    buildBotPrompt(historyContext, currentConfigContext) {
