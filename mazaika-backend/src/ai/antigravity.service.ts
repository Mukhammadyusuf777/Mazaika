...
import { generateComprehensiveProject, detectNiche } from './niche-synthesizer';
import { applySemanticOfflinePatch } from './offline-patch';
...
export class AntigravityService {
  ...
  async generateFullProject(
    ...
    try {
      ...
      // If NOT edit mode, do NOT pass old HTML or old projectState into new prompt context!
      if (!isEditMode) {
        existingHtml = '';
        projectState = null;
      }
...
Bot Edges: ${JSON.stringify(currentBotEdges.slice(0, 20))}`;
        ...
        if (siteData?.patches?.length > 0) {
          const patched = this.applyWebPatches(existingHtml, siteData.patches);
          if (patched !== existingHtml) {
            finalHtml = patched;
            patchApplied = true;
          }
        } else if (siteData?.html && siteData.html.length > 250) {
        ...
        // Guaranteed edit execution: if AI failed or returned untouched code, run Semantic Offline Patcher!
        const isRu = isRawRussian(promptText);
        if (!patchApplied || finalHtml === existingHtml) {
          this.logger.log('🛠️ Running Semantic Offline Patch Engine for guaranteed edit execution...');
          const offlineResult = applySemanticOfflinePatch(existingHtml, promptText, isRu);
          finalHtml = offlineResult.html;
          if (!siteData?.explanation || siteData.explanation === 'Muvaffaqiyatli yangilandi!') {
            siteData = { ...(siteData || {}), explanation: offlineResult.explanation };
          }
        }
        ...
        const isRu = isRawRussian(promptText);
        const richFallback = generateComprehensiveProject(promptText, target || 'bot_and_mini_app', isRu);
        ...
        const finalResult = {
          ...
          title: appName,
          appName: appName,
          explanation: siteData?.explanation || botData?.explanation || (
            isRu
        ...
        if (finalResult.html) {
          const translit = (str: string) => {
            const ruMap: Record<string, string> = { 'а':'a','б':'b','в':'v','г':'g','д':'d','е':'e','ё':'yo','ж':'zh','з':'z','и':'i','й':'y','к':'k','л':'l','м':'m','н':'n','о':'o','п':'p','р':'r','с':'s','т':'t','у':'u','ф':'f','х':'kh','ц':'ts','ч':'ch','ш':'sh','щ':'shch','ъ':'','ы':'y','ь':'','э':'e','ю':'yu','я':'ya' };
  ...
  private generateRichFallbackProject(promptText: string, target: string, isRu: boolean) {
    return generateComprehensiveProject(promptText, (target === 'site_only' ? 'site_only' : 'bot_and_mini_app'), isRu);
  }
}
