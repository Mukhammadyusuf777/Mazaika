...
export class WorkflowService {
  ...
  private async executeNodeAction(ctx: any, node: any, contactId: string, variables: any, botId: string, edges: any[], bot?: any): Promise<{wait: boolean, stateUpdates?: any}> {
    try {
      if (node.type === 'start') {
      ...
      if (node.type === 'stars') {
        ...
        try {
          // Native Telegram Stars invoice: provider_token is empty string, currency is XTR
          await ctx.replyWithInvoice({
            title,
            description,
      ...
      if (node.type === 'photo') {
        ...
        if (url) {
          try {
            ...
            if (caption) await ctx.reply(caption, extra);
          }
        } else if (caption) {
      ...
      if (node.type === 'aiImage') {
        ...
        // Pollination AI dynamic image generator (100% free, $0.00 cost)
